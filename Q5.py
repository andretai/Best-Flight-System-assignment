...
"""
def countplot(listcountry):
    coun_name=[]
    coun_stop=[]
    coun_word=[]

    for news in listcountry.items():
        coun_stop.append(sum(news.get_sum("stopword") for news in newspapers))
        coun_word.append(sum(news.get_sum("word") for news in newspapers))


    trace1=go.Bar( x=coun_name,y=coun_word,name='Word Count')
    trace2=go.Bar(x=coun_name,y=coun_stop,name='Stop Count')
...
for c in city:
    ...
    stopCount[c] = countWord(newClass.getWord())
